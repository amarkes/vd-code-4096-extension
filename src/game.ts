...
export class Game {
  ...
  private slideRow(row: number[]): { result: number[]; score: number; changed: boolean } {
    ...
    if (!changed) {
      for (let j = 0; j < 5; j++) {
        if (result[j] !== row[j]) { changed = true; break; }
      }
    }
  ...
  private checkGameOver(): void {
    ...
    for (let r = 0; r < 5; r++) {
      for (let c = 0; c < 5; c++) {
        ...
        if (c + 1 < 5 && this.board[r][c + 1] === v) { return; }
        if (r + 1 < 5 && this.board[r + 1][c] === v) { return; }
      }
    }
    this.gameOver = true;
  }
